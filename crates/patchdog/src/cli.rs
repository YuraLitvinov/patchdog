...
/// Asynchronously sends a collection of `Request` objects to the Google Gemini API
/// and collects the corresponding responses.
///
/// This function manages a pool of requests, prepares them for batched sending,
/// dispatches them to the API, and then matches the received responses back to
/// their original requests. It includes logic to retry requests that did not
/// receive a valid response in the initial batch.
///
/// # Arguments
/// - `request`: A `Vec<Request>` containing the list of requests to be processed.
///
/// # Returns
/// - `Result<Vec<ResponseForm>, ErrorBinding>`: A `Result` containing a vector
///   of `ResponseForm` objects, where each form includes the original request
///   data and the new comment generated by the API, or an `ErrorBinding` if
///   any part of the process (e.g., API communication, response parsing) fails.
...
/// Writes new comments into their respective source files based on a collection of `ResponseForm` objects. ///   The responses are sorted in descending order by line number to ensure correct insertion without invalidating subsequent line indices. ///   For each response, the function reads the target file, converts its content into a vector of lines, inserts the `new_comment` at the specified line, and then persists the modified content back to the file.
